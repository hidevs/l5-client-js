...
export default class l5Client {
    ...
    search(key: string, operator: string, value: string): this {
        this._queryParams[Filter.QueryParamKeys.SearchFields] =
            (this._queryParams[Filter.QueryParamKeys.SearchFields] || "") + `${key}:${operator};`;
        this._queryParams[Filter.QueryParamKeys.Search] = (this._queryParams[Filter.QueryParamKeys.Search] || "") + `${key}:${value};`;
        return this;
    }
    ...
    order(direction: Filter.OrderDirection, column: string, joinTable?: string): this;
    order(direction: Filter.OrderDirection, column: string, joinTable?: string, joinColumn?: string): this;
    order(direction: Filter.OrderDirection, column: string, joinTable?: string, joinColumn?: string): this {
        if (joinTable === undefined && joinColumn === undefined) {
            ...
            this._queryParams[Filter.QueryParamKeys.OrderBy] = (
                (this._queryParams[Filter.QueryParamKeys.OrderBy] || "") + `${joinTable}|${column};`
            ).replace(/^\;?|\;?$/g, "");
        } else if (joinTable !== undefined && joinColumn !== undefined) {
            // 3. order by join table column and specify join column
            this._queryParams[Filter.QueryParamKeys.OrderBy] = (
                (this._queryParams[Filter.QueryParamKeys.OrderBy] || "") + `${joinTable}:${joinColumn}|${column};`
            ).replace(/^\;?|\;?$/g, "");
        }
